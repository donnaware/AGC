...
void MEM::writeMemory(unsigned address, unsigned data)
{
	...
	if(bank == 0) {
		if(lowAddress > 1024) {
			MonitorForm1->Memo2->Lines->Add("Error: Eraseable address=" + AnsiString(lowAddress));
			return;
		}
		MEM::register_EMEM[lowAddress].write(data);
		...
		if((highAddress | lowAddress) > 1024*12) {
			MonitorForm1->Memo2->Lines->Add("Error: Fixed address=" + AnsiString( (highAddress | lowAddress) ));
			return; 
		}
