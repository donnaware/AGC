...
//---------------------------------------------------------------------------
// Performs an inclusive OR or register U and register C; in the MASK instruction,
// the RC and RU control pulses are activated simultaneously. This causes both to be
...
{
/*
    unsigned test_mask  = outmask() + 1;
    unsigned test_Xreg  = ALU::register_X.read();
    unsigned test_Yreg  = ALU::register_Y.read();
    unsigned test_CIreg = ALU::register_CI.read();
    unsigned test_COreg = (outmask()+1) & (ALU::register_X.read() + ALU::register_Y.read());
    unsigned test_Ureg  = outmask() & (ALU::register_X.read() + ALU::register_Y.read() + test_COreg);


    AnsiString tmp;
    tmp =         "X=0x" + IntToHex((int)test_Xreg,4);
    tmp = tmp +  " Y=0x" + IntToHex((int)test_Yreg,4);
    tmp = tmp +  " U=0x" + IntToHex((int)test_Ureg,4);
    tmp = tmp + " CO=0x" + IntToHex((int)test_COreg,4);
    tmp = tmp + " CI=0x" + IntToHex((int)test_CIreg,4);
    MonitorForm1->Memo2->Lines->Add(tmp);
*/

	unsigned carry = (outmask()+1) & (ALU::register_X.read() + ALU::register_Y.read()); // end-around carry
	if(carry || ALU::register_CI.read()) carry = 1;
	else                                 carry = 0;
